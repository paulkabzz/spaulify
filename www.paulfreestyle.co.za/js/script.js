...
function loadMusic(i) {
  ...
  document.getElementById(
    "player-image"
  ).src = `${ALBUMS_DIR}${musicArray[i].img}`;
  ...
  try {
    ...
    image.addEventListener("load", function () {
      ...
      const updateHeaderStyle = (dominantColor) => {
        ...
        const isDarkColor = dominantColor.every(
          (value) => value < darkColorThreshold
        );
        ...
        if (scrollY > 390) {
          ...
          headerOverlay.style.backgroundColor =
            scrollY < 390 ? "transparent" : "rgb(0 0 0 / 0.45)";
	...
	// Incase unable to load dominant color from album image, default to black background color.
...
function playMusic() {
  ...
  playPauseButton.forEach(
    (button) => (button.innerHTML = '<i class="fa-solid fa-circle-pause"></i>')
  );
...
function nextSong() {
  if (isRepeat) {
    ...
    musicIndex > musicArray.length - 1
      ...
      : (musicIndex = musicIndex);
    loadMusic(musicIndex);
    document.querySelector(".repeat-image").src =
      "../assets/images/ui/not-repeat.png";
  } else if (isShuffle) {
    let newIndex;
    do {
...
repeatButton.onclick = () => {
  ...
  document.querySelector(".repeat-image").src = isRepeat
    ? "../assets/images/ui/repeated.png"
    : "../assets/images/ui/not-repeat.png";
};
...
shuffleButton.onclick = () => {
  ...
  document.querySelector(".shuffle-image").src = isShuffle
    ? "../assets/images/ui/shuffle.png"
    : "../assets/images/ui/not-shuffle.png";
};
